...
def test_get_contratos_by_ug_success(cliente_contratos):
    # Configura o mock para retornar uma resposta bem-sucedida
    mock_response = Mock()
    mock_response.status_code = HTTPStatus.OK
    mock_response.json.return_value = [{"id": 1, "ug_code": "12345"}]

    with patch(
        "airflow_lappis.plugins.cliente_contratos.ClienteBase.request",
        return_value=(HTTPStatus.OK, mock_response.json.return_value),
    ):
        # Chama o método get_contratos_by_ug
        result = cliente_contratos.get_contratos_by_ug("12345")

        # Verifica se o resultado é o esperado
        assert result == [{"id": 1, "ug_code": "12345"}]
...
def test_get_contratos_by_ug_invalid_data(cliente_contratos):
    mock_response = Mock()
    mock_response.status_code = HTTPStatus.OK
