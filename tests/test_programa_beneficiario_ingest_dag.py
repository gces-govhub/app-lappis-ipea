...
from airflow_lappis.dags.data_ingest.programa_beneficiario_ingest_dag import dag_instace as dag
...
def test_fetch_and_store_programa_beneficiario_success(
    ...
    assert sorted(inserted_data, key=lambda x: x["id_programa"]) == sorted(expected_data, key=lambda x: x["id_programa"])
